...
install_nodejs() {
  ...
  if needs_resolution "$version"; then
    ...
    local version=$(curl --silent --get --retry 5 --retry-max-time 15 --data-urlencode "range=${version}" https://semver.scalingo.io/node/resolve)
  fi
...
install_iojs() {
  ...
  if needs_resolution "$version"; then
    ...
    version=$(curl --silent --get  --retry 5 --retry-max-time 15 --data-urlencode "range=${version}" https://semver.scalingo.io/iojs/resolve)
  fi
...
install_npm() {
  ...
  if [ "$version" == "" ]; then
    ...
    if needs_resolution "$version"; then
      ...
      version=$(curl --silent --get --retry 5 --retry-max-time 15 --data-urlencode "range=${version}" https://semver.scalingo.io/npm/resolve)
    fi
