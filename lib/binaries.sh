...
install_nodejs() {
  ...
  curl "$download_url" --retry 3 --silent --fail -o /tmp/node.tar.gz || (echo "Unable to download node $version; does it exist?" && false)
...
install_iojs() {
  ...
  curl "$download_url" --retry 3 --silent --fail -o /tmp/node.tar.gz || (echo "Unable to download iojs $version; does it exist?" && false)
  tar xzf /tmp/node.tar.gz -C /tmp
