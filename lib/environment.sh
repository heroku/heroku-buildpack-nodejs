...
export_env_dir() {
    local env_dir=$1
    if [ -d "$env_dir" ]; then
        local whitelist_regex=${2:-'^(BUILD_ENV|FIREBASE_API_KEY|MAP_BOX_KEY|IS_PREVIEW_LINK|ONHAND_TOOLKIT_NPM_TOKEN|PACKAGE_VERSION|RECAPTCHA_SITE_KEY|RECAPTCHA_SECRET_KEY)$'}
